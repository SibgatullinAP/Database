...
int DataBase::selectingRequest(Command &current, FILE* outFile)
{
	...
	Container->HeadSelect = select;

	switch (current.getPrintSort())
	{
	    case PS_NONE:
		break;
	    ...
	    case PS_PHONE:
		Container->sortByPhone();
		break;

	    case PS_GROUP:
		Container->sortByGroup();
		break;
	}

	select = Container->HeadSelect;
	while(select)
	{
	    select->print();
	    select = select->NextSelect;
...
void DataBase::run(FILE* inputRequest, FILE *outputResult)
{
	...
	//current.print(0, true); //True if is for Debug_Print, Else for string that have been parsed.
